...
# https://github.com/LucaOne/LucaPCycle/blob/master/src/common/pooling.py

def get_pooling_layer(pooling_method, embed_size):
    pooling_map = {
...
class GlobalMaskWeightedAttentionPooling1D(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        if mask is not None:
            attention_probs = nn.Softmax(dim=-1)(logits + (1.0 - mask.float()) * -10000)
        else:
...
class GlobalMaskValueAttentionPooling1D(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        if mask is not None:
            attention_probs = nn.Softmax(dim=1)(e + torch.unsqueeze((1.0 - mask.float()) * -10000, dim=-1))
        else:
            attention_probs = nn.Softmax(dim=1)(e)
